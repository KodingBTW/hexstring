...
class CLI:
    ...
    def show_help(self):
        ...
        print("  --ignore-end-lines  Ignore end lines (optional)")
    ...
    def handle_extract(self):
        ...
        if self.args.use_split_pointers is not None:
            print(f"Using split pointers metrod, lsb offset: {self.args.use_split_pointers[0]}, msb offset: {self.args.use_split_pointers[1]}, size: {self.args.use_split_pointers[2]}")
        
        if self.args.ignore_end_lines:
            print("Ignoring end lines.")
        ...
        # Decode Script
        if self.args.no_use_end_lines:
            script, total_bytes_read, lines_lenght = Decoder.decode_script_no_end_line(rom_data, format_pointers, self.args.no_use_end_lines + 1, char_table, bracket_index)
        else:
            end_line = Decoder.parse_end_lines(self.args.end_line)
            ignore_end_lines_code_until_decode = self.args.ignore_end_lines         
    ...
    def handle_insert(self):
        ...
        # Encode Text
        if self.args.no_use_end_lines:
            new_script_data, new_script_size, cumulative_lenghts = Encoder.encode_script(new_script, None, char_table, longest_char, self.args.no_use_end_lines, bracket_index)
        else:
            new_script_data, new_script_size, cumulative_lenghts = Encoder.encode_script(new_script, end_line, char_table, longest_char, self.args.no_use_end_lines, bracket_index)
        ...
        if self.args.use_split_pointers is not None:
            ...
            if self.args.p == '2b':
                new_pointers_data, new_pointers_size = Encoder.calculate_pointers_2_bytes(cumulative_lenghts, original_text_start_offset, base, 0)
                pointers_lenght = 2
            elif self.args.p == '2bb':
                new_pointers_data, new_pointers_size = Encoder.calculate_pointers_2_bytes(cumulative_lenghts, original_text_start_offset, base, 1)
                pointers_lenght = 2
            elif self.args.p == '3b':
                new_pointers_data, new_pointers_size = Encoder.calculate_pointers_3_bytes(cumulative_lenghts, original_text_start_offset, base, 0)
                pointers_lenght = 3
            elif self.args.p == '3bb':
                new_pointers_data, new_pointers_size = Encoder.calculate_pointers_3_bytes(cumulative_lenghts, original_text_start_offset, base, 1)
                pointers_lenght = 3
            elif self.args.p == '4b':
                new_pointers_data, new_pointers_size = Encoder.calculate_pointers_4_bytes(cumulative_lenghts, original_text_start_offset, base, 0)
                pointers_lenght = 4
            elif self.args.p == '4bb':
                new_pointers_data, new_pointers_size = Encoder.calculate_pointers_4_bytes(cumulative_lenghts, original_text_start_offset, base, 1)
                pointers_lenght = 4
            else:
                print("\nError: Invalid pointers format!")
                exit(1)
    ...
    def setup_parser(self):
        parser = argparse.ArgumentParser(
            description="HexString CLI\n"
                    "Usage: %(prog)s [OPTIONS]"
        )

        # General
        parser.add_argument('-v', '--version', action='version', version="%(prog)s 1.3.0 by koda")
        ...
        extract_parser.add_argument('--ignore-end-lines', action='store_true', help='Ignore end lines')
